...
class Form extends React.Component {
  ...
  validateForm = (e) => {
    ...
    const isValidForm = !this.hasFormErrors(formId)
    this.props.onSubmit(e, this.values, this.formErrors, isValidForm)
  }
  ...
  handleBlur = (formId, inputName, isValid, inputStr) => {
    ...
    if (isValid) {
      if (formId) {
        ...
        this.values[inputName] = inputStr
        this.formErrors[inputName] = ''
      }
    } else if (formId) {
      this.formErrors[formId][inputName] = inputStr
    } else {
      this.formErrors[inputName] = inputStr
    }
    // e.preventDefault();
  };

  render() {
    const disabled = this.props.notallowed || this.props.disabled;
    const className = this.props.className;
    return (
      <div>
        <form method="post" disabled={disabled} id={this.props.id} onSubmit={this.validateForm} ref={this.props.onRef} className={className && className} noValidate>
          {this.props.children}
        </form>
      </div>
    )
...
export default authenticate(Form);