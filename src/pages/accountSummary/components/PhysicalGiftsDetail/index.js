...
class PhysicalGiftsDetail extends Component {
  constructor(props) {
    super(props)
    this.eventId = (this.props.params && this.props.params.eventId) ? this.props.params.eventId : ''
    this.pageType = 'physical'
    this.pageName = 'physicalgiftsdetail'
  ...
  render() {
    ...
    const MonthofPurchaseOptions = {
      ...
      options: [{
        option: commonUtil.getLabel(labels, 'accountStatement.physical.dropdownLabel1'),
        value: 'showAll',
        disabled: false,
        selected: false,
      }, {
    ...
    const allStores = {
      ...
      disable: false,
      title: commonUtil.getLabel(labels, 'accountStatement.physical.dropdownLabel2'),
      options: [{
        id: 'allStores',
        value: 'allStores',
        label: commonUtil.getLabel(labels, 'accountStatement.physical.dropdownLabel2'),
        disabled: false,
        isChecked: false,
        isAllSelector: true,
      }],
    ...
    const applyForBonusOptions = {
      ...
      options: [{
        option: commonUtil.getLabel(labels, 'accountStatement.physical.dropdownLabel3'),
        value: 'allBonus',
        disabled: false,
        selected: false,
      }, {
    ...
    const physicalGiftRow = function (data, index) {
      return (
          ...
          {!_.isEmpty(purchasedItemSummaryData) &&
            ...
            ?
              <React.Fragment>
                <div className="row">
                <div className="col-xs-12">
                  <h2 className="mainTitle">{commonUtil.getLabel(labels, 'accountStatement.physical.label1')}</h2>
                </div>
              </div>
                <div className="row">
                <div className="white-box">
                  <div className={isBonusContainerVisible ? 'col-xs-3 right-border text-right ph60' : 'col-xs-9 right-border text-right ph60'} > <span className="titleInfoDetail">{commonUtil.getLabel(labels, 'accountStatement.physical.label2')}</span> <span className="qtyDetail">{purchasedItemSummaryData.itemSummary.totalPurchasedItemsCount}</span></div>
                  <div className={isBonusContainerVisible ? 'col-xs-3 right-border text-right ph60' : 'col-xs-3 text-right ph60'}><span className="titleInfoDetail">{commonUtil.getLabel(labels, 'accountStatement.physical.label3')}</span><span className="qtyDetail">{commonUtil.getCurrency(purchasedItemSummaryData.itemSummary.totalPurchasedAmount)}</span></div>
                  {isBonusContainerVisible ? <React.Fragment>
                    <div className="col-xs-3 right-border text-right ph60"><span className="titleInfoDetail">{commonUtil.getLabel(labels, 'accountStatement.physical.label4')}</span><span className="qtyDetail">{commonUtil.getCurrency(purchasedItemSummaryData.itemSummary.totalAmountEligibleForBonus)}</span>
                    ...
                    <div className="col-xs-3 text-right ph60">
                      <i className="iClass icon-ayuda tooltip-container" >
                        <span className="tooltip-box">{commonUtil.getLabel(labels, 'accountStatement.physical.tooltipMessage')}</span>
                      </i>&nbsp;&nbsp; <span className="text">{commonUtil.getLabel(labels, 'accountStatement.physical.label6')}</span>
                  ...
                  <div className="text-right">
                    <div className="right m-30">
                      <div className="account-iconWrap">
                        ...
                        <PrintDownload footer="ESTADO DE CUENTA" brand={this.brand || ''} elem=".downloadPrintPage" useDefault uiname="AccountStatementDownload" fileName="accountStatement.pdf" usePageHeader="false" />
              ...
              <React.Fragment>
                ...
                <div className="col-xs-12 bv nph">
                  <div className={isBonusContainerVisible ? 'custom-grid-6 nmv' : 'custom-grid-5 nmv'}>
                    <div className="centeredh"><span onClick={() => this.onSortChange('date', 'ascending')}>{commonUtil.getLabel(labels, 'accountStatement.physical.columnLabel1')}{this.state.sortingParameters.length && this.state.sortingParameters[0].type === 'date' && this.state.sortingParameters[0].value === 'descending' ? sortDownIcon : sortUpIcon}</span></div>
                    <div className="centeredh"><span>{commonUtil.getLabel(labels, 'accountStatement.physical.columnLabel2')}</span></div>
                    <div className="centeredh"><span onClick={() => this.onSortChange('amount', 'ascending')}>{commonUtil.getLabel(labels, 'accountStatement.physical.columnLabel3')}{this.state.sortingParameters.length && this.state.sortingParameters[0].type === 'amount' && this.state.sortingParameters[0].value === 'descending' ? sortDownIcon : sortUpIcon}</span></div>
                    <div className="centeredh"><span>{commonUtil.getLabel(labels, 'accountStatement.physical.columnLabel4')}</span></div>
                    <div className="centeredh"><span onClick={() => this.onSortChange('store', 'ascending')}>{commonUtil.getLabel(labels, 'accountStatement.physical.columnLabel5')}{this.state.sortingParameters.length && this.state.sortingParameters[0].type === 'store' && this.state.sortingParameters[0].value === 'descending' ? sortDownIcon : sortUpIcon}</span></div>
                    {isBonusContainerVisible ? <div className="centeredh"><span onClick={() => this.onSortChange('bonusApplicable', 'ascending')}>{commonUtil.getLabel(labels, 'accountStatement.physical.columnLabel6')}{this.state.sortingParameters.length && this.state.sortingParameters[0].type === 'bonusApplicable' && this.state.sortingParameters[0].value === 'descending' ? sortDownIcon : sortUpIcon}</span></div> : null
                    }
                  </div>
                </div>
              </div>
                <div className="container-results">
                {purchasedItemsDetailsData.itemDetails.map(physicalGiftRow)}
              </div>
            ...
            : <React.Fragment>
              {!_.isEmpty(purchasedItemsDetailsData)
                && (purchasedItemsDetailsData.status) ?
                  <p className="error"> {purchasedItemsDetailsData.status.errorMessage}</p> : ''}
            </React.Fragment>}
        </div>
      </div >
    )
...
export default PhysicalGiftsDetail
